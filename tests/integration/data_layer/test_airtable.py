...
def test_airtable_integration_list(airtable_client):
...
@pytest.mark.integration
def test_airtable_integration_get(airtable_client):
    records = airtable_client.get(id='recdCdKEoExUm5bjs')
    assert records is not None
    assert isinstance(records, ServiceAtLocationResponse)

@pytest.mark.integration
def test_airtable_integration_list_with_filters(airtable_client):
...
@pytest.mark.integration
def test_airtable_bulk_get(airtable_client):
